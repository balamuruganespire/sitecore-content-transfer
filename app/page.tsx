...
const DEFAULT_SOURCE_HOST = process.env.NEXT_PUBLIC_SOURCE_CM_HOST || "";
const DEFAULT_TARGET_HOST = process.env.NEXT_PUBLIC_TARGET_CM_HOST || "";
const OVERRIDE_SESSION_STORAGE =
  process.env.NEXT_PUBLIC_OVERRIDELOCALSTORAGE === "true";
...
const INITIAL_STEPS: RunStep[] = [
  ...
  {
    id: "status",
    label: "Wait for source to package content",
    state: "pending",
  },
...
export default function Home() {
  const [source, setSource] = useState<EnvCredentials>({
    ...EMPTY_ENV,
    host: DEFAULT_SOURCE_HOST,
  });
  const [target, setTarget] = useState<EnvCredentials>({
    ...EMPTY_ENV,
  ...
  // Prefill Client ID/Secret from server-side env vars (SOURCE_/TARGET_CLIENT_ID/SECRET),
  // without clobbering anything the user already typed or restored from the session.
  useEffect(() => {
    fetch("/api/env-defaults")
      ...
      .then((data) => {
        ...
        setSource((prev) => ({
          ...prev,
          clientId: prev.clientId || data.source?.clientId || "",
          clientSecret: prev.clientSecret || data.source?.clientSecret || "",
        }));
        setTarget((prev) => ({
          ...prev,
          clientId: prev.clientId || data.target?.clientId || "",
          clientSecret: prev.clientSecret || data.target?.clientSecret || "",
        }));
      })
      .catch(() => {
        /* env defaults unavailable */
      });
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (!OVERRIDE_SESSION_STORAGE) {
          if (saved.source) {
            setSource((prev) => ({
              ...saved.source,
              host: saved.source.host || prev.host,
            }));
          }
          if (saved.target) {
            setTarget((prev) => ({
              ...saved.target,
              host: saved.target.host || prev.host,
            }));
          }
        }
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ source, target, database, trees }),
      );
    } catch {
  ...
  const validTrees = trees.filter((t) =>
    t.ItemPath.trim().startsWith("/sitecore/"),
  );
  const ready =
    !running &&
    source.host &&
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        <p className="hint">
          One row per content tree. Paths exactly as shown in the Content
          Editor, e.g. <code>/sitecore/content/MySite/Home</code>.
        </p>
        {trees.map((tree, i) => (
          <div className="tree-row" key={i}>
            ...
            <select
              ...
              disabled={running}
              onChange={(e) =>
                updateTree(i, { Scope: e.target.value as Scope })
              }
            >
              {SCOPES.map((s) => (
      ...
      <div className="card">
        ...
        <p className="hint">
          The full pipeline runs step by step: initiate → package → relay chunks
          → assemble .raif → consume → verify.
        </p>
        <ol className="steps">
          {steps.map((s) => (
            <li key={s.id}>
        ...
        {done && warnings.length === 0 && (
          <div className="success">
            ✓ Transfer complete — content is now in the target environment.
            Review it in the Content Editor and publish when ready.
          </div>
        )}
        {done && warnings.length > 0 && (
          <div className="warning">
            ⚠ Transfer finished with errors — the .raif was consumed and content
            should be in the target, but some items reported issues. Verify them
            in the target Content Editor (common causes: templates or referenced
            items missing on the target — include them in the transfer, or use
            &quot;Item + descendants&quot;).
            {warnings.map((w, i) => (
